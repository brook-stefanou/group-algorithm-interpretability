...
gap is the headline claim, C1.

## Evidence grades

Every claim below carries an evidence grade.  Rung 0 is an association
...
mechanism claims (C2, C3, C5) live inside single models and reach higher.
...
orders 21–255.  The rule claiming eleven of them was fixed before any run.
...
noise floor, so no occupancy claim is made there; its endpoints are
...
Everything that is run is reported: the claim set filters what is
confirmatory, and a null outcome on any claim ships as a measurement.
...
for large effects, and the headline claim should be powered for more than the
...
pair — the direction that flattens a difference claim.  C5 keeps n = 50
because a mechanistic claim needs seed replication: the reported quantity is
...
- Eleven pairs are eleven pairs, and two of them are embedding probes of a
  ...
  two embedding probes, and no sampling claim over a population of groups is
  made.
...
- No absolute-difficulty claims are made on abelian groups; the transpose
